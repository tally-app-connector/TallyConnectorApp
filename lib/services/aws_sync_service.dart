...
class AwsSyncService {
  ...
  Future<void> initialize() async {
    ...
    try {
      _connection = await Connection.open(
        Endpoint(
          host: AwsAuroraConfig.host, // tally-cloud-sync-db-instance-1.cp8em48qwti1.ap-south-1.rds.amazonaws.com
          database: AwsAuroraConfig.database, // postgres
          username: AwsAuroraConfig.username, // tally_admin
          password: AwsAuroraConfig.password, // from AWS Secrets Manager
          port: AwsAuroraConfig.port, // 5432
        ),
        settings: ConnectionSettings(
          sslMode: SslMode.require,
          connectTimeout: Duration(seconds: AwsAuroraConfig.connectTimeout),
          queryTimeout: Duration(seconds: AwsAuroraConfig.queryTimeout),
        ),
      );
  ...
  /// Reconnect if connection was lost (Aurora may drop idle connections)
  ...
  Future<void> createCompaniesTable() async {
    ...
    try {
      // Ensure user_data schema exists
  ...
  Future<void> _createTablesInSchema(String schemaName) async {
    ...
    // ============================================
    // VOUCHER TYPES TABLE - EXACT MATCH WITH LOCAL
    // ============================================
    await _connection!.execute('''
...
Future<void> _syncVoucherTypeBatch(
  ...
  for (int i = 0; i < voucherTypes.length; i++) {
    ...
    final baseIndex = i * 21; // 21 fields

    final placeholders =
        List.generate(21, (j) => '\$${baseIndex + j + 1}').join(', ');
    valueClauses.add('($placeholders)');
  ...
  final sql = '''
    INSERT INTO $schemaName.voucher_types (
      company_guid, name, guid, parent, parent_guid, alter_id, master_id,
  ...
  Future<void> _syncVoucherBatchAllocationsBatch(
    ...
    INSERT INTO $schemaName.voucher_batch_allocations (
      ...
      amount, actual_qty, billed_qty, batch_id, tracking_number, mfg_date, expiry_date, batch_rate,
